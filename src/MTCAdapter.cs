...
namespace MTConnect
{
     ...
     public class Adapter
     {
          ...
          /// </summary>
          private readonly ArrayList mClients = new ArrayList();

          /// <summary>
          /// A count of client threads.
          /// </summary>
          private readonly CountdownEvent mActiveClients = new CountdownEvent(1);
          ...
          /// </summary>
          private readonly ArrayList mDataItems = new ArrayList();
          ...
          /// </summary>
          readonly ASCIIEncoding mEncoder = new ASCIIEncoding();
          ...
          public void AddAsset(Asset asset)
          {
               ...
               DateTime now = DateTime.UtcNow;
               _ = result.Append(now.ToString("yyyy-MM-dd\\THH:mm:ss.fffffffK"));
               _ = result.Append("|@ASSET@|");
               _ = result.Append(asset.AssetId);
               _ = result.Append('|');
               _ = result.Append(asset.GetMTCType());
               _ = result.Append("|--multiline--ABCD\n");
          ...
          public void SendToAll(string line)
          {
               ...
               foreach (Stream client in mClients.ToArray().Cast<Stream>())
               {
                    lock (client)
                    {
          ...
          private void HeartbeatClient(object client)
          {
               ...
               try
               {
                    while (mRunning && tcpClient.Connected)
                    {
                         ...
                         {
                              readList.Clear();
                              _ = readList.Add(tcpClient.Client);
          ...
          public void Stop()
          {
               if (mRunning)
               {
                    ...
                    _ = mListenThread.Join(2 * Heartbeat);
